...
export function createApi(uri: string, networkId: NetworkID, opts: ApiOptions = {}): NetworkApi {
  ...
  return {
    loadProposalVotes: async (
      ...
      const addresses = data.votes.map((vote: { voter: { id: any; }; }) => vote.voter.id);
      const names = await getNames(addresses);

      return data.votes.map((vote: { voter: { name: string | null; id: string | number; }; }) => {
    ...
    loadProposals: async (
      ...
      if (highlightApolloClient) {
        const { data: highlightData } = await highlightApolloClient.query({
          ...
          variables: { ids: data.proposals.map((proposal: { id: any; }) => proposal.id) }
        });

        data.proposals = data.proposals.map((proposal: { id: any; proposal_id?: number; metadata?: { id: string; title: string; body: string; discussion: string; execution: string; }; space?: { id: string; controller: string; metadata: { name: string; avatar: string; voting_power_symbol: string; executors: string[]; executors_types: string[]; }; authenticators: string[]; strategies_parsed_metadata: ApiStrategyParsedMetadata[]; }; author?: { id: string; }; quorum?: number; execution_hash?: string; start?: number; min_end?: number; max_end?: number; snapshot?: number; scores_1?: number; scores_2?: number; scores_3?: number; scores_total?: number; execution_time?: number; execution_strategy?: string; execution_strategy_type?: string; timelock_veto_guardian?: string | null; strategies_indicies?: number[]; strategies?: string[]; strategies_params?: any[]; created?: number; edited?: number | null; tx?: string; execution_tx?: string | null; veto_tx?: string | null; vote_count?: number; executed?: boolean; vetoed?: boolean; completed?: boolean; cancelled?: boolean; }) => {
          const highlightProposal = highlightData.sxproposals.find(
            (highlightProposal: any) => highlightProposal.id === proposal.id
          );

          return joinHighlightProposal(proposal as any, highlightProposal);
        });
      }

      return data.proposals.map((proposal: ApiProposal) => formatProposal(proposal, networkId, current));
    },
    loadProposal: async (
      spaceId: string,
      proposalId: number,
    ...
    loadSpaces: async (
      ...
      return data.spaces.map((space: ApiSpace) => formatSpace(space, networkId));
    },
